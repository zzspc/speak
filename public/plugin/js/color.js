...
color_htm += '</tr>';
color_htm += '</table>';
color_htm += '<div id="sharf_color_show"></div>';
function color_show(id, color, obj) {
	Eh();
	if($$('sharf_color') == null) {
		var sharf_color_div = document.createElement("div");
		with(sharf_color_div.style) {zIndex = 9999; position = 'absolute'; display = 'none'; width = '160px'; padding = '1px'; top = 0; left = 0; border = '#A0A0A0 1px solid'; backgroundColor = '#FFFFFF';}
		sharf_color_div.id = 'sharf_color';
		document.body.appendChild(sharf_color_div);
	}
	var aTag = obj; var leftpos = toppos = 0;
	do {aTag = aTag.offsetParent; leftpos	+= aTag.offsetLeft; toppos += aTag.offsetTop;
	} while(aTag.offsetParent != null);
	$$('sharf_color').style.left = (obj.offsetLeft + leftpos) + 'px';
	$$('sharf_color').style.top = (obj.offsetTop + toppos + 20) + 'px';
	$$('sharf_color').innerHTML = color_htm;
	color_id = id;
	color_bk = color;
	$$('sharf_color').style.display = '';
	if(color) color_view(color);
	color_setup(color);
}
function color_hide() {hide('sharf_color'); Es();}
...
function color_setup(color) {
	...
	colors_select += '</table>';
	$$('sharf_color_show').innerHTML = colors_select;
}
